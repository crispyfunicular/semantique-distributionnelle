...
### Biais coloniaux mise en évidence par GloVe dans le corpus de départ
...
la hiérarchisation coloniale qui imprègne l’oeuvre de Kipling et Conrad, confirmant notre hypothèse de départ.
...
Ces biais font écho au *les Damnés de la terre*, essai dans lequel Franz Fanon explique comment le monde colonial est divisé en deux :
...
Par conséquent, bien que la méthode masque l'asymétrie en surface, la géométrie globale de l'espace vectoriel demeure intacte. Ainsi, Gonen et Goldberg ont pu, en utilisant un simple classificateur d'apprentissage automatique sur des vecteurs “neutralisés”, prédire le biais d'origine des mots avec une précision extrêmement élevée (entre 88,88 % et 96,53 %), montrant ainsi qu'il est possible de retrouver le biais, même enfoui après application d'une méthode de *hard debiasing* (Gonen et Goldberg, 2019).
...
Ces résultats invitent à une réflexion méthodologique plus large : **le biais colonial dans ce corpus n’est pas un artefact de déséquilibre lexical — c’est une structure sémantique profonde, inscrite dans la façon même dont les mots cooccurrent**. Les deux méthodes testées s’attaquent à des symptômes superficiels (fréquences d’occurrence, géométrie locale de l’espace vectoriel) sans toucher aux réseaux de sens qui organisent le corpus. Un débiaisage véritablement efficace nécessiterait sans doute une intervention plus radicale sur le corpus lui-même — sélection de textes contre-hégémoniques, réécriture assistée — ou le recours à des techniques d’alignement inter-corpus avec des textes postcoloniaux.


## Conclusion
Ce résultat est d’autant plus remarquable qu’il reproduit un topos de la pensée coloniale victorienne (l’opposition esprit/corps, raison/instinct) documenté par la critique postcoloniale (Said, 1978 ; Bhabha, 1994).
